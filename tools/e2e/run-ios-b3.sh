#!/usr/bin/env bash
# tools/e2e/run-ios-b3.sh — the session-live E2E driver (W-SESS leg;
# b3.session.live, decision D9): the FULL upstream agent spine boots
...
# one-to-one manifest b3-session-live.json. Screenshots are saved artifacts
# (screens/) — the verdict is logs only.
#
# usage: run-ios-b3.sh [--udid U] [--art-dir D] [--skip-build]
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: run-ios-b3.sh [--udid U] [--art-dir D] [--skip-build]" >&2; exit 2 ;;
  esac
done
[ -n "$ART" ] || ART="hosts/ios/artifacts/b3-session-live"
LOG="$ART/logs.txt"
mkdir -p "$ART" "$ART/screens"

log() { echo "run-ios-b3: $*"; }
die() { echo "run-ios-b3: FAIL: $*" >&2; exit 1; }
...
fail_deadline() {
  echo "run-ios-b3: DEADLINE EXPIRED: $*" >&2
...
log "5/5 launch (session-live mode; log capture truncated — checker sees only this run)"
...
xcrun simctl launch --terminate-running-process \
  ...
  "$UDID" "$APP_BUNDLE_ID" -dsh-mode session-live >/dev/null
...
if node tools/e2e/check.mjs --manifest tools/e2e/scenarios/b3-session-live.json \
    --log "$LOG" --out "$ART/verdict-b3-session-live.json"; then
  echo "==================== E2E summary ($ART) ===================="
  echo "  b3-session-live           PASS"
  echo "  logs: $LOG  screens: $ART/screens/"
else
  echo "==================== E2E summary ($ART) ====================" >&2
  echo "  b3-session-live           FAIL" >&2
  echo "  logs: $LOG  screens: $ART/screens/" >&2
  die "b3-session-live checker failed — see $ART/verdict-b3-session-live.json"
fi
