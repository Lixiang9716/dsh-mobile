/**
 * M5 host-binding scenario `harmony.capability-binding` — runs on the HarmonyOS host
 ...
 * Client in ArkWeb. Modeled on android-capability-binding.js (the Android twin): after
 ...
 * tools/e2e/scenarios/harmony-capability-binding.json; the carrier's own events
 * (listening / mounted / connected / token deltas / session complete) ride
 * the same canonical stream from the host side as scenario
 * `harmony.capability-binding` records (module `dsh.carrier`), so ONE manifest covers
