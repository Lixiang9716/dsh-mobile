#!/bin/sh
# run-live-llm.sh — the REAL-backend LLM E2E on the HarmonyOS emulator
# (scenario `llm.live-stream`, real leg; the W-HARMONY5 leg that closes M5). The twin
# of hosts/android/ci/run-live-llm.sh, with this host's staging direction
...
#      (`aa start ... --ps dsh.e2e.leg llm.live-stream` — EntryAbility.onCreate parks
...
#   3. verify the pulled capture against llm-live-stream-device.json AND
#      llm-live-stream-carrier.json, then grep the RAW hilog stream and the RAW
...
# and its capture is committed under artifacts/m5-llm-live-stream/ as the honest
...
#
# usage: [DSH_SKIP_BUILD=1] hosts/harmony/ci/run-live-llm.sh [artifacts-dir]
set -eu
...
while :; do
    STREAM=/tmp/dsh-harmony-llm-live-stream-hilog.txt
    [ "$attempt" -gt 1 ] && STREAM=/tmp/dsh-harmony-llm-live-stream-hilog.$attempt.txt
    ...
    "$HDC" file recv "$CAPTURE_REMOTE" /tmp/dsh-llm-live-stream-attempt-capture.txt >/dev/null 2>&1 || true
    if grep -q '"event":"llm.stream.started"' /tmp/dsh-llm-live-stream-attempt-capture.txt 2>/dev/null; then
        kill "$streamer" 2>/dev/null || true
        streamer=""
...
"$HDC" shell snapshot_display -f /data/local/tmp/dsh-llm-live-stream.jpeg >/dev/null 2>&1 || true
"$HDC" file recv /data/local/tmp/dsh-llm-live-stream.jpeg "$OUT/llm-live-stream-live-page.jpeg" >/dev/null 2>&1 \
    || say "screenshot unavailable (non-fatal)"
if [ -f "$OUT/llm-live-stream-live-page.jpeg" ]; then
    sips -s format png "$OUT/llm-live-stream-live-page.jpeg" --out "$OUT/llm-live-stream-live-page.png" >/dev/null
    rm -f "$OUT/llm-live-stream-live-page.jpeg"
fi
...
node test/e2e/check.mjs --manifest test/e2e/scenarios/llm-live-stream-device.json \
    --log "$OUT/capture.txt" --out "$OUT/verdict-llm-live-stream-device.json" || true
