/**
 * M2 real-LLM scenario `m2.llm` — one streaming chat turn against an
 * OpenAI-compatible backend THROUGH the gateway, closing the "mock LLM"
 * gap the m2.session scenario left open. The scenario is 100%
 ...
 *     `m2-llm/config.json` in fs scope "app" ({baseUrl, apiKey, model} —
 ...
 *     is under test in CI.
 *
 * Event shapes are the m2.session vocabulary, byte-for-byte: agent.started,
...
const log = createLogger('m2.llm.spike');
...
const main = async () => {
  ...
  project({ kind: 'slot.register', id: 'notes.toolbar', label: 'llm', by: 'm2-llm' });
