#!/bin/sh
# run-m2-llm.sh — the REAL-backend LLM E2E on the Android emulator (scenario
# `m2.llm`, real leg). Sibling of hosts/android/ci/run-spike-e2e.sh (same
...
#      (files/profiles/default/m2-llm/config.json via run-as — the key is
...
#      `m2.llm`, entry scenario/m2-llm.js, one real streaming chat turn
#      through the gateway httpFetch);
#   3. verify the captured log against m2-llm-device.json AND
#      m2-llm-carrier.json, then grep the raw stream for the API key —
...
OUT=${DSH_M2_LLM_OUT:-hosts/android/artifacts/m2-llm}
STREAM=$OUT/dsh-m2-llm-stream.txt
...
adb shell "run-as $PKG sh -c 'mkdir -p files/profiles/default/m2-llm && cat > files/profiles/default/m2-llm/config.json'" < "$CFG"
adb shell "run-as $PKG cat files/profiles/default/m2-llm/config.json" | grep -q '"apiKey"' \
...
until grep -q "dsh.spike.result: ALL" "$STREAM"; do
    if [ "$(date +%s)" -ge "$deadline" ]; then
        echo "::error::m2.llm session did not complete within 300s"
        tail -80 "$STREAM"
        exit 1
    fi
    sleep 0.2
