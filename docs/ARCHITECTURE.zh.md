...
- **M4/M5**：Android、鸿蒙宿主。M4 进行中：同构宿主已验证 —— `m2.session`（三个系统插件上的首个 MINI 智能体会话）、网关桥接场景（`m2.bridge.smoke`）与 `m1.spike.boot` 回归全部在模拟器上通过（证据 `hosts/android/artifacts/m4-host/`）。
