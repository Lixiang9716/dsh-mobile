...
- **M4/M5**: Android and HarmonyOS hosts. M4 DONE: the completion session `m4.host-binding` (35/35) runs green on the emulator — the loopback carrier serves the embedded Web Client into a real WebView with the session rendered live (slot acked, token deltas, session-complete), and the nine-primitive gateway binding is real (descriptor 9 available / 0 unavailable; the mandatory audit re-verified via `m2.gateway.audit` 16/16), with the three-scenario regression green in the same run (evidence `hosts/android/artifacts/m4-complete/`).
...
- **M4/M5**: Android and HarmonyOS hosts — M4 done, M5 in progress. The isomorphic hosts run all three spike scenarios in one launch on their emulators (evidence `hosts/android/artifacts/m4-host/`, `hosts/harmony/artifacts/m5-host/`); Android additionally runs the completion session `m4.host-binding` 35/35 — carrier + WebView mount + the real nine-primitive binding (evidence `hosts/android/artifacts/m4-complete/`).
