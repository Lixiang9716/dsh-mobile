...
- **M4/M5**: Android and HarmonyOS hosts. M4 in progress: the isomorphic host is verified — `m2.session` (the first MINI agent session over the three system plugins), the gateway-bridge scenario (`m2.bridge.smoke`), and the `m1.spike.boot` regression all pass on the emulator (evidence `hosts/android/artifacts/m4-host/`).
