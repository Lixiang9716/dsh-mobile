...
<<<<<<< HEAD
- **M3 pluginization**: IN PROGRESS — the install pipeline is verified as a receipt transaction (`m3.install` 21/21 on the macOS CLI: content-addressed blob → trust-record verify → strict manifest validation → staged integrity read-back → receipt commit; a tampered package is rejected before unpack — evidence `runtime/spike/artifacts/macos-cli-m3-install/`), and the first two UI-plugin levels are verified on device (config-selected Web Client swap `m3.ui-swap` 7/7 to the mini variant + the plugin toolbar slot registered, rendered and acked live — evidence `hosts/ios/artifacts/m3-pluginization/`). Still open: the fetch-based installer with pending-receipt startup replay, capability negotiation at install time, the remaining UI-plugin level; the real LLM API remains open.
- **M4/M5**: Android and HarmonyOS hosts, both DONE — each isomorphic host runs the headless regression trio (`m1.spike.boot` 7/7, `m2.bridge.smoke` 6/6, `m2.session` 23/23) AND the event-driven binding phase in one launch. M4: loopback carrier + WebView mount + real nine-primitive binding (Keystore-sealed keychain, SAF directory picker + fsScope persist/resolve, notification + `notify.response`, `app.state` edges) — `m4.host-binding` 35/35 (evidence `hosts/android/artifacts/m4-complete/`). M5 **DONE**: the isomorphic HarmonyOS host runs the regression trio (`m1.spike.boot` 7/7, `m2.bridge.smoke` 6/6, `m2.session` 23/23) headlessly AND the event-driven binding phase in one launch — the loopback carrier (HTTP static serving of the Web Client + RFC 6455 WS pump via the shared bus seam) mounts the Web Client in ArkWeb with token deltas streaming live, the real binding primitives are proven on-device (notify + notification-tap `notify.response`, presentApproval dialog, fsScope app-scope persist/resolve, `app.state` lifecycle edges) and the honest `unavailable` set (presentPicker/keychainGet/keychainSet/httpFetch) is descriptor-declared — `m5.host-binding` 20/20 by one-to-one log-compare (evidence `hosts/harmony/artifacts/m5-host/`).
- **M3 pluginization**: install pipeline (receipt transactions) + the three UI-plugin levels + capability negotiation.
=======
- **M3 pluginization**: DONE — install = a receipt transaction over the frozen fs primitives (`m3.install` 22/22 on the macOS CLI: content-addressed blob → trust-record verify → strict manifest validation → install-time capability negotiation against the RuntimeDescriptor → staged integrity read-back → receipt commit; a tampered package is rejected before unpack — evidence `runtime/spike/artifacts/macos-cli-m3-install/`). The fetch-based installer runs the streaming httpFetch body through the same pipeline: on the CLI via a logged scope-read stub (`m3.complete` 41/41, `runtime/spike/artifacts/macos-cli-m3-complete/`) and ON DEVICE against the loopback carrier itself with the real `httpFetch` (`m3.fetch-install` 46/46 + carrier evidence `m3.fetch-carrier` 11/11, `hosts/ios/artifacts/m3-complete/`), including pending-receipt STARTUP REPLAY over the append-only receipt journal (a staged tree that verifies completes to committed; an interrupted unpack rolls back with the installed tree untouched). All three UI-plugin levels are verified on device: the config layer (`cordis.patch` layered overrides, JSON in the spike) selects the active Web Client and overrides the toolbar slot allow-set (the denied slot never renders), and the whole-client swap + component slot legs are green (`m3.ui-swap` 7/7, `hosts/ios/artifacts/m3-pluginization/`). Spike-local boundaries (§12): packages are uncompressed ustar (no inflate primitive on the frozen gateway), unpack atomicity is approximated by stage → verify → promote anchored by the receipt journal (no rename primitive), and the config patch is JSON rather than YAML. The real LLM API remains open (M2).
...
- **M4/M5**: Android and HarmonyOS hosts, both in progress. The isomorphic hosts run all three spike scenarios in one launch on their emulators — `m2.session` (the first MINI agent session over the three system plugins), `m2.bridge.smoke`, and the `m1.spike.boot` regression (evidence `hosts/android/artifacts/m4-host/`, `hosts/harmony/artifacts/m5-host/`).
>>>>>>> 63b9123 (feat(hosts/ios): M3 on-device fetch-install E2E — carrier self-hosted package, config-selected client, startup replay)
...
The following are **declared unsupported** on this host (flagged via capability negotiation, never faked): the real-subprocess ecosystem (bash/git hooks/playwright/python PTC/SSH/Windows ACL), desktop-grade background residency (the ceiling is checkpoint/resume + notification wake), whole-disk file access (the ceiling is user-granted security-scoped directories). Spike-local install boundaries (deliberate, revisited when the real profile host lands): packages are uncompressed ustar — the frozen gateway has no inflate primitive, so a gzip transport coding is a candidate minor addition; unpack "atomicity" is approximated by stage → verify → promote anchored by the append-only receipt journal (no rename primitive); the `cordis.patch` config layer is JSON rather than YAML (no parser primitive; semantics are format-independent).
