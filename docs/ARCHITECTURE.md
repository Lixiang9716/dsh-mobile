...
- **M4/M5**: Android and HarmonyOS hosts. M5 **DONE**: the isomorphic HarmonyOS host runs the regression trio (`m1.spike.boot` 7/7, `m2.bridge.smoke` 6/6, `m2.session` 23/23) headlessly AND the event-driven binding phase in one launch — the loopback carrier (HTTP static serving of the Web Client + RFC 6455 WS pump via the shared bus seam) mounts the Web Client in ArkWeb with token deltas streaming live, the real binding primitives are proven on-device (notify + notification-tap `notify.response`, presentApproval dialog, fsScope app-scope persist/resolve, `app.state` lifecycle edges) and the honest `unavailable` set (presentPicker/keychainGet/keychainSet/httpFetch) is descriptor-declared — `m5.host-binding` 20/20 by one-to-one log-compare (evidence `hosts/harmony/artifacts/m5-host/`).
