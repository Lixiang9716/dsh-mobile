...
## D11 — Release-please authenticates with GITHUB_TOKEN, and dispatches its own required check

- **Decision**: supersedes D10's `RELEASE_PLEASE_TOKEN` clause. release-please runs with `secrets.GITHUB_TOKEN`, and the workflow produces the `gates` check the release PR needs by **dispatching the `gov` workflow on the release branch** (`.github/workflows/gov.yml` gains a `workflow_dispatch` trigger; `release/please` calls `gh workflow run gov.yml --ref <release-branch>` as its last step, failing loud if the dispatch is refused). `workflow_dispatch` is a documented exception to GitHub's suppression of workflow events created by `GITHUB_TOKEN`, so the check is a real gate run on the real commit — no bypass, and nothing fabricated through the Checks API. `main` requires only the `gates` context, from the Actions app, which a dispatched run of that workflow supplies. The `RELEASE_PLEASE_TOKEN` secret is no longer read by any workflow; it is left in place, unread.
- **Alternatives**:
  - Keep the fine-grained PAT (`RELEASE_PLEASE_TOKEN`) and add `pull requests: write`: rejected — it makes the pipeline depend on a credential that fails in a way nothing local can detect. The failure it produced is instructive: the PAT authenticated, carried `contents: write`, let the action create its branch and commit, and died only at its final `POST /pulls`, so the version stream was dead for days while every visible signal looked healthy. This repo has now paid that cost once; a credential the workflow does not need cannot be misconfigured.
  ...
  - Put the PAT into a step output and pass it to the action, falling back to `GITHUB_TOKEN` when it cannot open pull requests: rejected — a working PAT would additionally start the three platform pipelines on the release PR, but that is a preview rather than a verification (`release/packages` builds and checks all three hosts on the published event), and it is not worth two credential paths whose PAT branch cannot be tested from here.
