...
- **Decision**: supersedes D10's `RELEASE_PLEASE_TOKEN` clause. release-please runs with `secrets.GITHUB_TOKEN`, and the required repository setting **"Allow GitHub Actions to create and approve pull requests"** (`can_approve_pull_request_reviews`) is ON — without it `POST /pulls` is refused outright with *"GitHub Actions is not permitted to create or approve pull requests"*, which is the failure the PAT had been masking. That setting is the operative fix: with it on, the release PR is created by `GITHUB_TOKEN` and **its `pull_request` workflows run normally** — measured on PR #107, where `gov` and all three platform pipelines (`dev/ios`, `dev/android`, `dev/harmonyos`) all reported green checks from `pull_request` events, and the PR reached `CLEAN`/mergeable with no operator action. As a guarantee rather than a hope, `release/please` also **dispatches the `gov` workflow on the release branch** (`.github/workflows/gov.yml` gains `workflow_dispatch`; the last step runs `gh workflow run gov.yml --ref <release-branch>` and fails loud if refused): `workflow_dispatch` is a documented exception to the suppression of `GITHUB_TOKEN`-created events, so the `gates` check protection actually requires exists even if the PR-event path does not fire. That check is a real gate run on the real commit — never a check fabricated through the Checks API. The `RELEASE_PLEASE_TOKEN` secret is no longer read by any workflow; it is left in place, unread.
- **Alternatives**:
  ...
  - Rely on the `pull_request` event alone and drop the dispatch: rejected — the dispatch costs one ~20s run, it is the only path whose behaviour does not depend on suppression semantics, and it guarantees the single check `main` requires. The PR-event path is what supplies the platform pipelines, which protection does not require.
