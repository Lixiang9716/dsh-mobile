...
| M4 | Android host (QuickJS-isomorphic) | In progress (isomorphic host verified: `m2.session` (first MINI agent session over the three system plugins) plus the gateway-bridge scenario `m2.bridge.smoke` and the `m1.spike.boot` regression all green on the emulator — evidence `hosts/android/artifacts/m4-host/`) |
