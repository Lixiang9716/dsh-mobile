...
<<<<<<< HEAD
| M3 | Plugin install pipeline + UI pluggability (slot / Web Client swap) | In progress (install pipeline verified as a receipt transaction on the macOS CLI — `m3.install` 21/21: content-addressed blob, trust-record verify, strict manifest validation, staged integrity read-back, receipt commit, tampered package rejected before unpack with the installed tree untouched — evidence `runtime/spike/artifacts/macos-cli-m3-install/`; UI pluggability verified on device — config-selected swap to the `dsh-web-client-mini` variant (`m3.ui-swap` 7/7) and the dsh-notes toolbar slot rendered + acked live — evidence `hosts/ios/artifacts/m3-pluginization/`; still open: fetch-based installer + pending-receipt replay, remaining UI-plugin levels, real LLM API) |
...
| M4 | Android host (QuickJS-isomorphic) | In progress (isomorphic host verified: `m2.session` — the first MINI agent session over the three system plugins — plus the gateway-bridge scenario `m2.bridge.smoke` and the `m1.spike.boot` regression, all green on the emulator in one launch; evidence `hosts/android/artifacts/m4-host/`) |
| M5 | HarmonyOS host (ArkTS + NAPI) | In progress (isomorphic host verified: `m2.session` over the three system plugins green on the emulator alongside `m1.spike.boot` + `m2.bridge.smoke` in one launch — evidence `hosts/harmony/artifacts/m5-host/`) |
