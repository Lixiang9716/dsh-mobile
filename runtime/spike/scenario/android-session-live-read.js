...
/**
 * android-session-live-read.js — the ON-DEVICE SESSION-LIVE runtime half behind
 * android.session.live-read (decision D9, the Android W-DROID leg): the Android
 * sibling of session-web-live.js — the FULL upstream agent spine boots inside the
 * embedded runtime (upstream/boot.js — ctx.sessions / agents / agentLoop /
 * tools / systemPrompt / sessionProjections / settings and the vendored
 * dsh-llm LlmRuntime), and the web-boot producer (upstream/web-boot.js)
 * mounts the OFFICIAL boot wire on the SAME context — so the carrier's
 * `/api/session.list` and the mux `session/journal` streams are CLAIMED and
 * answered with REAL data from the spine. Nothing is faked: endpoints the
 * spine does not implement stay structured-unavailable.
 *
 * The composition is platform-neutral (upstream/boot.js + web-boot.js are
 * shared verbatim with the CLI proof runs and the iOS b3 leg); only the
 * scenario id, the session identity, and one Android transport fact differ:
 * logcat truncates lines at ~4KB, so `runtime.booted` emits `entryCount`
 * instead of the 58-id list (the iOS sibling logs the full array — no such
 * cap there).
 *
...
const SCENARIO = 'android.session.live-read';
const AGENT_ID = 'main';
const SESSION_ID = 's-android-live-0001';
...
const log = createLogger('android.session');
...
const main = async () => {
  ...
  log.debug('b-android session-live runtime resident (claims live; awaiting the page attach)', {});
};
