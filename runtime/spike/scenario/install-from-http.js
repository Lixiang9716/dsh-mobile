/**
 * M3 on-device fetch-install scenario `install.from-http` — the M3 scope
 ...
 *     case is proven on the CLI in install.full-cycle);
 *   - the config layer patch (profiles/install-full-cycle/cordis.patch.json) is
 *     consumed host-side BEFORE eval: it selected the ACTIVE Web Client
 *     and the toolbar slot allow-set (carrier-side evidence,
 *     scenario install.carrier-evidence); the scenario projects one slot the
 *     configured set DENIES and the one it admits.
 *
 * Session phase is session.mock-llm-shaped: the host.info readiness signal
 ...
 * separate scenario install.carrier-evidence so the two one-to-one checkers never
 * interleave.
 */
...
const RESULT_PATH = 'install-from-http/result.txt';
const RESULT_TEXT = 'install-from-http result: 5 deltas'; // 33 ASCII bytes
...
/** Task steps (compute + persist) — same shape as session.mock-llm. */
...
/** The session phase (session.mock-llm-shaped): token deltas as an event
