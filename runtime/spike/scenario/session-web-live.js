...
/**
 * session-web-live.js — the ON-DEVICE SESSION-LIVE runtime half behind
 * session.live-read (decision D9, the W-SESS leg): the FULL upstream agent
 * spine boots inside the embedded runtime (upstream/boot.js — ctx.sessions /
 * agents / agentLoop / tools / systemPrompt / sessionProjections / settings
 * and the vendored dsh-llm LlmRuntime), and the web-boot producer
 * (upstream/web-boot.js) mounts the OFFICIAL boot wire on the SAME context —
 * so the carrier's `/api/session.list` and the mux `session/journal` streams
 * are CLAIMED and answered with REAL data from the spine. Nothing is faked:
 * endpoints the spine does not implement stay structured-unavailable.
 *
 ...
 *   host → runtime : runtime.config {mockLlmUrl, apiKey}, web.plugins,
 *                    api.request, mux.open, mux.cancel
...
const SCENARIO = 'session.live-read';
const AGENT_ID = 'main';
const SESSION_ID = 's-b3-ondevice-0001';
...
const main = async () => {
  ...
  log.debug('b3 runtime resident (claims live; awaiting the page attach)', {});
};
