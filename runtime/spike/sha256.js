 ...
 * ~90-line dependency-free implementation keeps install a pure data
 * operation over the fs primitives. Bytes in (Uint8Array), lowercase hex out.
 */
const K = [
...
/** 64-bit big-endian bit length into the block tail. JS shifts count mod 32,
 * so the high word comes from division — `bitLen >>> 32` would repeat the
 * low word and corrupt every 8th schedule word. */
const writeBitLength = (buf, bitLen) => {
  const hi = Math.floor(bitLen / 0x100000000);
  const lo = bitLen >>> 0;
  const at = buf.length - 8;
  for (let i = 0; i < 4; i++) {
    buf[at + i] = (hi >>> (24 - 8 * i)) & 0xff;
    buf[at + 4 + i] = (lo >>> (24 - 8 * i)) & 0xff;
  }
};
