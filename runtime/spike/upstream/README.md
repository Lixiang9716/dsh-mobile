...
| upstream-suite gap-fill (2026-09-25, the first full 262-spec sweep) | `Buffer.from` gains the single-byte family (ascii/latin1/binary); `fileURLToPath` resolves scheme-less RELATIVE paths (the transpiled specs' `import.meta.url` spelling) against the bundle root; `node:fs` gains loud link stubs `mkdtempSync`/`symlinkSync` (ESM named imports are link errors even when unreached); `node:fs/promises` gains `rmdir` (workspace-rm delegation — no empty-dir bookkeeping) and `symlink` (loud: the workspace link seam is hard-link only); `shims/util.js`'s TextDecoder now imports the `decodeUtf8` its non-fatal branch always called — a LATENT PRODUCT BUG the sweep exposed (any non-fatal decode crashed) | supported (the sweep's honest boundary: fixture staging for the preset specs and symlink/realpath semantics in the staged fs stay open) |
