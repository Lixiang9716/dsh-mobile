...
# m2.session scenario to complete — NO UI interaction: the scenario
...
#   - m2-session.json         (the JS scenario's 22 events)
#   - m2-webclient-mount.json (carrier-side mount/connection/projection)
...
log() { echo "run-ios-session: $*"; }
die() { echo "run-ios-session: FAIL: $*" >&2; exit 1; }
...
fail_deadline() {
  echo "run-ios-session: DEADLINE EXPIRED: $*" >&2
