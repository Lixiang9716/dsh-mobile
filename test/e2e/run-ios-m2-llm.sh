...
# (Documents/profiles/default/m2-llm/config.json — from the environment
# ZAI_BASE_URL / ZAI_API_KEY / ZAI_MODEL, or the repo-root .env), launches
# the app in session mode with `-dsh-scenario m2-llm`, waits for the Web
# Client mount + the streamed deltas + the terminal marker, then verifies
# the captured log against BOTH manifests:
#   - m2-llm-device.json  (the JS scenario: real leg, deltas, served model,
#                          key audit)
#   - m2-llm-carrier.json (mount/connection/projection evidence)
...
#
# usage: run-ios-m2-llm.sh [--udid U] [--art-dir D] [--skip-build]
#
...
ART="hosts/ios/artifacts/m2-llm"
SKIP_BUILD=0
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: run-ios-m2-llm.sh [--udid U] [--art-dir D] [--skip-build]" >&2; exit 2 ;;
  esac
done
LOG="$ART/logs.txt"
mkdir -p "$ART" "$ART/screens"

log() { echo "run-ios-m2-llm: $*"; }
die() { echo "run-ios-m2-llm: FAIL: $*" >&2; exit 1; }
...
fail_deadline() {
  echo "run-ios-m2-llm: DEADLINE EXPIRED: $*" >&2
...
: "${ZAI_BASE_URL:?run-ios-m2-llm: ZAI_BASE_URL missing (env or .env)}"
: "${ZAI_API_KEY:?run-ios-m2-llm: ZAI_API_KEY missing (env or .env)}"
: "${ZAI_MODEL:?run-ios-m2-llm: ZAI_MODEL missing (env or .env)}"
...
# (FSPrimitives.swift); the scenario reads m2-llm/config.json from there.
CONTAINER="$(xcrun simctl get_app_container "$UDID" "$APP_BUNDLE_ID" data)"
CFG_DIR="$CONTAINER/Documents/profiles/default/m2-llm"
mkdir -p "$CFG_DIR"
umask 077
...
log "4/5 launch (-dsh-scenario m2-llm; log capture truncated — checker sees only this run)"
...
xcrun simctl launch --terminate-running-process \
  ...
  "$UDID" "$APP_BUNDLE_ID" -dsh-mode session -dsh-scenario m2-llm >/dev/null
...
for s in m2-llm-device m2-llm-carrier; do
