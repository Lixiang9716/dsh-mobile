#!/usr/bin/env bash
# test/e2e/run-ios-live-write.sh — the session-write E2E driver (W-RPC leg; D9):
...
#
# usage: run-ios-live-write.sh [--udid U] [--art-dir D] [--skip-build]
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: run-ios-live-write.sh [--udid U] [--art-dir D] [--skip-build]" >&2; exit 2 ;;
  esac
done
[ -n "$ART" ] || ART="hosts/ios/artifacts/composer-live-write"
...
case "$ART" in
  /tmp/*|/private/tmp/*)
    echo "run-ios-live-write: FAIL: --art-dir under /tmp writes into the SIMULATOR's" >&2
...
log() { echo "run-ios-live-write: $*"; }
die() { echo "run-ios-live-write: FAIL: $*" >&2; exit 1; }
...
fail_deadline() {
  echo "run-ios-live-write: DEADLINE EXPIRED: $*" >&2
...
if node test/e2e/check.mjs --manifest test/e2e/scenarios/composer-live-write.json \
  ...
  echo "  composer-live-write             PASS"
  echo "  logs: $LOG  screens: $ART/screens/"
else
  echo "==================== E2E summary ($ART) ====================" >&2
  echo "  composer-live-write             FAIL" >&2
