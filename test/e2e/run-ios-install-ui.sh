...
# session.mock-llm-shaped agent session. NO UI interaction: the scenario auto-runs
...
# manifests:
#   - install-from-http.json (the JS scenario's 45 events)
#   - install-carrier-evidence.json (carrier-side config/route/slot/mount evidence)
# Screenshots are saved artifacts (screens/) — the verdict is logs only
# (docs/ARCHITECTURE.md, "E2E verification").
#
# usage: run-ios-install-ui.sh [--udid U] [--art-dir D] [--skip-build]
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: run-ios-install-ui.sh [--udid U] [--art-dir D] [--skip-build]" >&2; exit 2 ;;
  esac
done
LOG="$ART/logs.txt"   # derived AFTER arg parsing — --art-dir must apply
mkdir -p "$ART" "$ART/screens"

log() { echo "run-ios-install-ui: $*"; }
die() { echo "run-ios-install-ui: FAIL: $*" >&2; exit 1; }
...
fail_deadline() {
  echo "run-ios-install-ui: DEADLINE EXPIRED: $*" >&2
...
log "4/5 launch (session mode, install-full-cycle profile; log capture truncated)"
...
xcrun simctl launch --terminate-running-process \
  ...
  "$UDID" "$APP_BUNDLE_ID" -dsh-mode session -dsh-profile install-full-cycle >/dev/null
...
run_check test/e2e/scenarios/install-from-http.json install-from-http
run_check test/e2e/scenarios/install-carrier-evidence.json install-carrier-evidence

echo "==================== E2E summary ($ART) ===================="
for s in install-from-http install-carrier-evidence; do
