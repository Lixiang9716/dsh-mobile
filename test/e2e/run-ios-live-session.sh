#!/usr/bin/env bash
# test/e2e/run-ios-live-session.sh — the session-live E2E driver (W-SESS leg;
# session.live-read, decision D9): the FULL upstream agent spine boots
...
#
# usage: run-ios-live-session.sh [--udid U] [--art-dir D] [--skip-build]
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: run-ios-live-session.sh [--udid U] [--art-dir D] [--skip-build]" >&2; exit 2 ;;
  esac
done
[ -n "$ART" ] || ART="hosts/ios/artifacts/session-live-read"
LOG="$ART/logs.txt"
mkdir -p "$ART" "$ART/screens"

log() { echo "run-ios-live-session: $*"; }
die() { echo "run-ios-live-session: FAIL: $*" >&2; exit 1; }
...
fail_deadline() {
  echo "run-ios-live-session: DEADLINE EXPIRED: $*" >&2
...
if node test/e2e/check.mjs --manifest test/e2e/scenarios/session-live-read.json \
  ...
  echo "  session-live-read           PASS"
  echo "  logs: $LOG  screens: $ART/screens/"
else
  echo "==================== E2E summary ($ART) ====================" >&2
  echo "  session-live-read           FAIL" >&2
  echo "  logs: $LOG  screens: $ART/screens/" >&2
  die "session-live-read checker failed — see $ART/verdict-session-live-read.json"
fi
