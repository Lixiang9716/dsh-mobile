#!/usr/bin/env bash
# test/e2e/run-ios-official-web-mount.sh — Phase-B "official web mounts" E2E driver, now
...
# log is verified against the one-to-one manifest officialweb-mount.json.
# Screenshots are saved artifacts (screens/) — the verdict is logs only.
#
# usage: run-ios-official-web-mount.sh [--udid U] [--art-dir D] [--skip-build]
set -euo pipefail
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "usage: run-ios-official-web-mount.sh [--udid U] [--art-dir D] [--skip-build]" >&2; exit 2 ;;
  esac
done
...
log() { echo "run-ios-official-web-mount: $*"; }
die() { echo "run-ios-official-web-mount: FAIL: $*" >&2; exit 1; }
...
fail_deadline() {
  echo "run-ios-official-web-mount: DEADLINE EXPIRED: $*" >&2
...
if node test/e2e/check.mjs --manifest test/e2e/scenarios/officialweb-mount.json \
  ...
  echo "  officialweb-mount     PASS"
  echo "  logs: $LOG  screens: $ART/screens/"
else
  echo "==================== E2E summary ($ART) ====================" >&2
  echo "  officialweb-mount     FAIL" >&2
  echo "  logs: $LOG  screens: $ART/screens/" >&2
  die "officialweb-mount checker failed — see $ART/verdict-officialweb-mount.json"
fi
